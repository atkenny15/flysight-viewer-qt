...
Genome::Genome(const QVector<double>& rhs) : QVector<double>(rhs) {
}

Genome::Genome(const Genome& p1, const Genome& p2, int k) {
    const int parts = 1 << k;
...
Genome::Genome(int genomeSize, int k, double minLift, double maxLift) {
    const int parts = 1 << k;
...
void Genome::mutate(int k, int kMin, double minLift, double maxLift) {
    const int parts = 1 << k;
...
MainWindow::DataPoints Genome::simulate(double h, double a, double c, double planformArea,
                                        double mass, const DataPoint& dp0, double windowBottom) {
    ...
    for (int i = 0; i < size(); ++i) {
        ...
        const double k1 = h * dtheta_dt(theta + k0 / 2, v + l0 / 2, x + m0 / 2, y + n0 / 2,
                                        (lift_prev + lift_next) / 2, planformArea, mass);
        const double l1 = h * dv_dt(theta + k0 / 2, v + l0 / 2, x + m0 / 2, y + n0 / 2,
                                    (drag_prev + drag_next) / 2, planformArea, mass);
        const double m1 = h * dx_dt(theta + k0 / 2, v + l0 / 2, x + m0 / 2, y + n0 / 2);
        const double n1 = h * dy_dt(theta + k0 / 2, v + l0 / 2, x + m0 / 2, y + n0 / 2);

        const double k2 = h * dtheta_dt(theta + k1 / 2, v + l1 / 2, x + m1 / 2, y + n1 / 2,
                                        (lift_prev + lift_next) / 2, planformArea, mass);
        const double l2 = h * dv_dt(theta + k1 / 2, v + l1 / 2, x + m1 / 2, y + n1 / 2,
                                    (drag_prev + drag_next) / 2, planformArea, mass);
        const double m2 = h * dx_dt(theta + k1 / 2, v + l1 / 2, x + m1 / 2, y + n1 / 2);
        const double n2 = h * dy_dt(theta + k1 / 2, v + l1 / 2, x + m1 / 2, y + n1 / 2);

        const double k3 =
            h * dtheta_dt(theta + k2, v + l2, x + m2, y + n2, lift_next, planformArea, mass);
        const double l3 =
            h * dv_dt(theta + k2, v + l2, x + m2, y + n2, drag_next, planformArea, mass);
...
double Genome::dtheta_dt(double theta, double v, double x, double y, double lift,
                         double planformArea, double mass) {
    // From https://en.wikipedia.org/wiki/Atmospheric_pressure#Altitude_variation
    const double airPressure = SL_PRESSURE * pow(1 - LAPSE_RATE * y / SL_TEMP,
...
double Genome::dv_dt(double theta, double v, double x, double y, double drag, double planformArea,
                     double mass) {
    // From https://en.wikipedia.org/wiki/Atmospheric_pressure#Altitude_variation
    const double airPressure = SL_PRESSURE * pow(1 - LAPSE_RATE * y / SL_TEMP,
                                                 A_GRAVITY * MM_AIR / GAS_CONST / LAPSE_RATE);
...
double Genome::dx_dt(double theta, double v, double x, double y) {
    return v * cos(theta);
}

double Genome::dy_dt(double theta, double v, double x, double y) {
    return v * sin(theta);
}

double Genome::lift(double cl) {
    return cl;
}

double Genome::drag(double cl, double a, double c) {
    return a * cl * cl + c;
}
