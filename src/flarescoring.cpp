...
FlareScoring::FlareScoring(MainWindow* mainWindow) :
    ScoringMethod(mainWindow), mMainWindow(mainWindow), mWindowBottom(1000) {
}

void FlareScoring::setWindowBottom(double windowBottom) {
    mWindowBottom = windowBottom;
    emit scoringChanged();
}

double FlareScoring::score(const MainWindow::DataPoints& result) {
...
QString FlareScoring::scoreAsText(double score) {
    return QString::number(score) + QString(" m");
}

void FlareScoring::prepareDataPlot(DataPlot* plot) {
    ...
    if (success && plot->yValue(DataPlot::Elevation)->visible()) {
        ...
        rect->bottomRight->setCoords(
            (plot->xValue()->value(dpBottom, mMainWindow->units()) - xMin) / (xMax - xMin), 1.1);
        ...
        rect->topLeft->setCoords(
            (plot->xValue()->value(dpTop, mMainWindow->units()) - xMin) / (xMax - xMin), -0.1);
...
bool FlareScoring::getWindowBounds(const MainWindow::DataPoints& result, DataPoint& dpBottom,
                                   DataPoint& dpTop) {
    int bottom, top;
    double hBottom, hTop;
    DataPoint dpPrev;
