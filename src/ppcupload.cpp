...
PPCUpload::PPCUpload(MainWindow* mainWindow, QObject* parent) : QObject(parent) {
    mMainWindow = mainWindow;
}

// HTTPS requires the OpenSSL binaries.
// On Windows these are libeay32.dll, libssl32.dll and ssleay32.dll available from
// https://slproweb.com/products/Win32OpenSSL.html

void PPCUpload::upload(const QString type, const double windowTop, const double windowBottom,
    ...
    post +=
        "Timestamp=" +
        mMainWindow->dataPoint(mMainWindow->findIndexBelowT(0.0)).dateTime.toString(Qt::ISODate) +
        "&";
...
void PPCUpload::finished(QNetworkReply* reply) {
    if (reply->error() == QNetworkReply::NoError) {
        QMessageBox(QMessageBox::Information, "Success", tr("Track submitted successfully."))
            .exec();
        QDesktopServices::openUrl(
            QUrl("https://ppc.paralog.net/showTrack.php&?track=" + reply->readAll()));
    }
    else
...
bool PPCUpload::getUserDetails(QString* name, QString* countrycode, bool getEqipment,
                               QString* equipment, QString* place) {
    ...
    if (getEqipment) {
        ...
        connect(naManager, SIGNAL(finished(QNetworkReply*)), this,
                SLOT(suitsFinished(QNetworkReply*)));
        naManager->get(request);
    }
    else {
    ...
    request = QNetworkRequest(
        QUrl("http://api.geonames.org/findNearbyPlaceNameJSON?lat=" + QString::number(p.lat) +
             "&lng=" + QString::number(p.lon) + "&username=flysight"));
    request.setRawHeader("User-Agent", "FlySight Viewer");
    request.setRawHeader("Charset", "utf8");
    naManager = new QNetworkAccessManager(this);
