...
**                                                                        **
**  FlySight Viewer                                                       **
**  Copyright 2018 Michael Cooper, Kenny Daniel                           **
**                                                                        **
...
public:
    RealItem(const QString& text, int type = Type) : QTableWidgetItem(text, type) {}
...
public:
    DurationItem(qint64 duration, int type = Type) :
        QTableWidgetItem(duration < 3600000 ? QString("%1:%2")
                                                  .arg(duration / 60000)
                                                  .arg((duration / 1000) % 60, 2, 10, QChar('0'))
                                            : QString("%1:%2:%3")
                                                  .arg(duration / 3600000)
...
LogbookView::LogbookView(QWidget* parent) :
    QWidget(parent), ui(new Ui::LogbookView), mMainWindow(0), suspendItemChanged(false) {
    ui->setupUi(this);

    connect(ui->tableWidget, SIGNAL(cellDoubleClicked(int, int)), this,
            SLOT(onDoubleClick(int, int)));
    connect(ui->tableWidget, SIGNAL(itemSelectionChanged()), this, SLOT(onSelectionChanged()));
    connect(ui->tableWidget, SIGNAL(itemChanged(QTableWidgetItem*)), this,
            SLOT(onItemChanged(QTableWidgetItem*)));
    connect(ui->searchEdit, SIGNAL(textChanged(QString)), this, SLOT(onSearchTextChanged(QString)));
    connect(ui->searchEdit, SIGNAL(returnPressed()), this, SLOT(onSearchTextReturn()));
}
...
void LogbookView::setMainWindow(MainWindow* mainWindow) {
...
void LogbookView::updateView() {
    ...
    ui->tableWidget->setColumnWidth(1,
                                    2 * ui->tableWidget->horizontalHeader()->minimumSectionSize());
    ...
    ui->tableWidget->setHorizontalHeaderLabels(
        QStringList() << tr("") << tr("") << tr("ID") << tr("File Name") << tr("Description")
                      << tr("Start Time") << tr("Duration") << tr("Sample Period")
                      << tr("Minimum Latitude") << tr("Maximum Latitude") << tr("Minimum Longitude")
                      << tr("Maximum Longitude") << tr("Import Time") << tr("Exit Time")
                      << tr("Ground Elevation") << tr("Course Angle") << tr("Wind Speed")
                      << tr("Wind Direction") << tr("Range Lower") << tr("Range Upper"));
    ...
    while (query.next()) {
        ...
        item->setCheckState(mMainWindow->trackChecked(query.value(1).toString()) ? Qt::Checked
                                                                                 : Qt::Unchecked);
        ...
        ui->tableWidget->setItem(index, 3,
                                 new QTableWidgetItem(query.value(1).toString())); // file_name
        ui->tableWidget->setItem(index, 4,
                                 new QTableWidgetItem(query.value(2).toString())); // description
        ...
        ui->tableWidget->setItem(index, 7,
                                 new RealItem(query.value(5).toString())); // sample_period
        ...
        ui->tableWidget->setItem(index, 14,
                                 new RealItem(QString::number(ground, 'f', 3))); // ground
        ui->tableWidget->setItem(index, 15,
                                 new RealItem(QString::number(course, 'f', 5))); // course
        ui->tableWidget->setItem(index, 16,
                                 new RealItem(QString::number(windSpeed, 'f', 2))); // wind_speed
        ui->tableWidget->setItem(index, 17,
                                 new RealItem(QString::number(windDir, 'f', 5))); // wind_dir
...
void LogbookView::onDoubleClick(int row, int column) {
    Q_UNUSED(column);
...
void LogbookView::onItemChanged(QTableWidgetItem* item) {
    ...
    if (item->column() == 1) {
        // Update check state
        mMainWindow->setTrackChecked(nameItem->text(), item->checkState() == Qt::Checked);
    }
...
void LogbookView::onSearchTextChanged(const QString& text) {
    Q_UNUSED(text);
    updateView();
}
