...
float Nav::dtor(float degrees) {
    return (degrees * M_PI / 180);
}

// Convert radians to degrees
float Nav::rtod(float radians) {
    return (radians * 180.0 / M_PI);
}

// Round float to integer
int32_t Nav::round_nearest(float f) {
    return (int32_t)(f + 0.5);
}

// Calculate relative bearing given bearing and heading in degrees
int Nav::calcRelBearing(int bearing, int heading) {
    int relBearing;
...
int Nav::calcBearing(float lat1, float lon1, float lat2, float lon2) {
    float bearing = 0.0;
...
// Heading in degrees
int Nav::calcDirection(const Config& config, int32_t lat, int32_t lon, int32_t head) {
    ...
    dLat = dtor((float)config.UBX_lat / GeoCo_Scale);
    dLon = dtor((float)config.UBX_lon / GeoCo_Scale);
    // convert heading
...
int32_t Nav::calcDistanceRad(float lat1, float lon1, float lat2, float lon2) {
    float dlon, dlat, a, c;
    float dist = 0.0;
...
// Calculate distance
int32_t Nav::calcDistance(int32_t lat1, int32_t lon1, int32_t lat2, int32_t lon2) {
    float cLat, cLon, dLat, dLon;
    int32_t dist;
    const int32_t GeoCo_Scale = 10000000;
