...
DataPlot::DataPlot(QWidget* parent) :
    QCustomPlot(parent), mMainWindow(0), m_dragging(false), m_xAxisType(Time),
    m_cursorValid(false) {
    // Initialize window
...
DataPlot::~DataPlot() {
    ...
    // Save plot state
    foreach (PlotValue* v, m_yValues) { v->writeSettings(); }
...
void DataPlot::initPlot() {
    ...
    foreach (PlotValue* v, m_xValues) { v->readSettings(); }
    ...
    foreach (PlotValue* v, m_yValues) { v->readSettings(); }
}
...
void DataPlot::mouseReleaseEvent(QMouseEvent* event) {
    if (mMainWindow->dataSize() > 0) {
        ...
        if (m_dragging && tool == MainWindow::Zoom) {
            QCPRange range(qMin(m_tBegin, xAxis->pixelToCoord(endPos.x())),
                           qMax(m_tBegin, xAxis->pixelToCoord(endPos.x())));
...
void DataPlot::wheelEvent(QWheelEvent* event) {
    if ((mMainWindow->dataSize() > 0) && axisRect()->rect().contains(event->pos())) {
        ...
        range = QCPRange(x + (range.lower - x) * multiplier, x + (range.upper - x) * multiplier);

        setRange(range);
    }
}

void DataPlot::leaveEvent(QEvent*) {
    if (mMainWindow->dataSize() > 0) {
        mMainWindow->clearMark();
        m_cursorValid = false;
        update();
...
void DataPlot::setMark(double start, double end) {
    if (mMainWindow->dataSize() == 0)
        return;
    ...
    if (dpStart.dateTime.date() == dpEnd.dateTime.date()) {
        ...
        status =
            QString("<p style='color:black;' align='center'><u>%1 %2.%3 to %4 %5.%6 UTC</u></p>")
                .arg(dpStart.dateTime.toUTC().date().toString(Qt::ISODate))
                .arg(dpStart.dateTime.toUTC().time().toString(Qt::ISODate))
                .arg(QString("%1").arg(dpStart.dateTime.time().msec(), 3, 10, QChar('0')))
    ...
    status += QString("<tr "
                      "style='color:black;'><td></td><td><u>Value</u></td><td><u>Change</u></"
                      "td><td><u>Min/Avg/Max</u></td></tr>");
    ...
    for (int i = 0; i < yaLast; ++i) {
        if (yValue(i)->visible()) {
            ...
            double avg = (yValue(i)->value(dpMin, mMainWindow->units()) +
                          yValue(i)->value(dpLow, mMainWindow->units())) /
                         2;
            ...
            for (int j = jMin; j < jMax; ++j) {
                ...
                avg = (yValue(i)->value(dp2, mMainWindow->units()) +
                       yValue(i)->value(dp1, mMainWindow->units())) /
                      2;
            ...
            avg = (yValue(i)->value(dpHigh, mMainWindow->units()) +
                   yValue(i)->value(dpMax, mMainWindow->units())) /
                  2;
            ...
            status += QString("<tr "
                              "style='color:%5;'><td>%1</td><td>%2</td><td>(%3%4)</td><td>[%6/%7/"
                              "%8]</td></tr>")
...
void DataPlot::setMark(double mark) {
    if (mMainWindow->dataSize() == 0)
        return;
    ...
    status +=
        QString(
            "<tr style='color:black;'><td align='center'><u>(%1 deg, %2 deg, %3 m)</u></td></tr>")
            .arg(dp.lat, 0, 'f', 7)
            .arg(dp.lon, 0, 'f', 7)
            .arg(dp.hMSL, 0, 'f', 3);
...
void DataPlot::setRange(const QCPRange& range) {
    if (mMainWindow->dataSize() == 0)
        return;
...
void DataPlot::updateYRanges() {
    ...
    for (int j = 0; j < yaLast; ++j) {
        ...
        if (!first) {
            ...
            axisRect()
                ->axis(QCPAxis::atLeft, k++)
                ->setRange(yValue(j)->useMinimum() ? yValue(j)->minimum() * factor : yMin,
...
void DataPlot::updatePlot() {
    ...
    for (int j = 0; j < yaLast; ++j) {
        ...
        QCPGraph* graph = addGraph(axisRect()->axis(QCPAxis::atBottom), axis);
        graph->setData(x, y);
        ...
        if (yValue(j)->hasOptimal()) {
            ...
            graph->setPen(
                QPen(QBrush(yValue(j)->color()), mMainWindow->lineThickness(), Qt::DotLine));
        }
    }
...
DataPoint DataPlot::interpolateDataX(double x) {
...
int DataPlot::findIndexBelowX(double x) {
    int below = -1;
...
int DataPlot::findIndexAboveX(double x) {
    int below = -1;
    int above = mMainWindow->dataSize();
...
void DataPlot::togglePlot(YAxisType plot) {
    m_yValues[plot]->setVisible(!m_yValues[plot]->visible());
    updatePlot();
}

void DataPlot::setXAxisType(XAxisType xAxisType) {
    if (mMainWindow->dataSize() == 0)
        return;
