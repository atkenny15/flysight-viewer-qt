...
MainWindow::MainWindow(QWidget* parent) :
    ...
    QMainWindow(parent), m_ui(new Ui::MainWindow), mMarkActive(false), m_viewDataRotation(0),
    m_units(PlotValue::Imperial), mWindowMode(Actual), mScoringView(0), m_mass(70),
    m_planformArea(2), m_minDrag(0.05), m_minLift(0.0), m_maxLift(0.5), m_maxLD(3.0),
    m_simulationTime(120), mLineThickness(0), mWindE(0), mWindN(0), mWindAdjustment(false),
    mScoringMode(PPC), mGroundReference(Automatic), mFixedReference(0), mMapMode(Default),
    mUseDatabase(true) {
    m_ui->setupUi(this);
    ...
    for (int i = PPC; i < smLast; ++i) {
        connect(mScoringMethods[i], SIGNAL(scoringChanged()), this, SIGNAL(dataChanged()));
    }
...
void MainWindow::initPlot() {
    ...
    connect(this, SIGNAL(dataChanged()), m_ui->plotArea, SLOT(updatePlot()));
    connect(this, SIGNAL(rangeChanged()), m_ui->plotArea, SLOT(updateRange()));
    connect(this, SIGNAL(cursorChanged()), m_ui->plotArea, SLOT(updateCursor()));
    connect(this, SIGNAL(mediaCursorChanged()), m_ui->plotArea, SLOT(updateCursor()));
}
...
void MainWindow::initSingleView(const QString& title, const QString& objectName,
    ...
    connect(actionShow, SIGNAL(toggled(bool)), dockWidget, SLOT(setVisible(bool)));
    connect(dockWidget, SIGNAL(visibilityChanged(bool)), actionShow, SLOT(setChecked(bool)));

    connect(this, SIGNAL(dataChanged()), dataView, SLOT(updateView()));
    connect(this, SIGNAL(rangeChanged()), dataView, SLOT(updateView()));
    connect(this, SIGNAL(cursorChanged()), dataView, SLOT(updateCursor()));
    connect(this, SIGNAL(mediaCursorChanged()), dataView, SLOT(updateCursor()));
    connect(this, SIGNAL(rotationChanged(double)), dataView, SLOT(updateView()));

    connect(dockWidget, SIGNAL(topLevelChanged(bool)), this,
            SLOT(onDockWidgetTopLevelChanged(bool)));
}
...
void MainWindow::initMapView() {
    ...
    connect(m_ui->actionShowMapView, SIGNAL(toggled(bool)), dockWidget, SLOT(setVisible(bool)));
    connect(dockWidget, SIGNAL(visibilityChanged(bool)), m_ui->actionShowMapView,
            SLOT(setChecked(bool)));

    connect(this, SIGNAL(dataLoaded()), mapView, SLOT(initView()));
    connect(this, SIGNAL(dataChanged()), mapView, SLOT(updateView()));
    connect(this, SIGNAL(rangeChanged()), mapView, SLOT(updateView()));
    connect(this, SIGNAL(cursorChanged()), mapView, SLOT(updateCursor()));
    connect(this, SIGNAL(mediaCursorChanged()), mapView, SLOT(updateCursor()));
    connect(this, SIGNAL(mapModeChanged()), mapView, SLOT(updateMapMode()));
...
void MainWindow::initWindView() {
    ...
    connect(m_ui->actionShowWindView, SIGNAL(toggled(bool)), dockWidget, SLOT(setVisible(bool)));
    connect(dockWidget, SIGNAL(visibilityChanged(bool)), m_ui->actionShowWindView,
            SLOT(setChecked(bool)));

    connect(this, SIGNAL(dataChanged()), windPlot, SLOT(updatePlot()));
    connect(this, SIGNAL(rangeChanged()), windPlot, SLOT(updatePlot()));
    connect(this, SIGNAL(cursorChanged()), windPlot, SLOT(updatePlot()));
    connect(this, SIGNAL(mediaCursorChanged()), windPlot, SLOT(updatePlot()));
...
void MainWindow::initScoringView() {
    ...
    connect(m_ui->actionShowScoringView, SIGNAL(toggled(bool)), dockWidget, SLOT(setVisible(bool)));
    connect(m_ui->actionShowScoringView, SIGNAL(toggled(bool)), this,
            SLOT(setScoringVisible(bool)));
    connect(dockWidget, SIGNAL(visibilityChanged(bool)), m_ui->actionShowScoringView,
            SLOT(setChecked(bool)));

    connect(this, SIGNAL(dataChanged()), mScoringView, SLOT(updateView()));
    connect(this, SIGNAL(rangeChanged()), mScoringView, SLOT(updateView()));
...
void MainWindow::initLiftDragView() {
    ...
    connect(m_ui->actionShowLiftDragView, SIGNAL(toggled(bool)), dockWidget,
            SLOT(setVisible(bool)));
    connect(dockWidget, SIGNAL(visibilityChanged(bool)), m_ui->actionShowLiftDragView,
            SLOT(setChecked(bool)));

    connect(this, SIGNAL(dataChanged()), liftDragPlot, SLOT(updatePlot()));
    connect(this, SIGNAL(rangeChanged()), liftDragPlot, SLOT(updatePlot()));
    connect(this, SIGNAL(cursorChanged()), liftDragPlot, SLOT(updatePlot()));
    connect(this, SIGNAL(mediaCursorChanged()), liftDragPlot, SLOT(updatePlot()));
    connect(this, SIGNAL(aeroChanged()), liftDragPlot, SLOT(updatePlot()));
...
void MainWindow::initOrthoView() {
    ...
    connect(m_ui->actionShowOrthoView, SIGNAL(toggled(bool)), dockWidget, SLOT(setVisible(bool)));
    connect(dockWidget, SIGNAL(visibilityChanged(bool)), m_ui->actionShowOrthoView,
            SLOT(setChecked(bool)));

    connect(this, SIGNAL(dataChanged()), orthoView, SLOT(updateView()));
    connect(this, SIGNAL(rangeChanged()), orthoView, SLOT(updateView()));
    connect(this, SIGNAL(cursorChanged()), orthoView, SLOT(updateView()));
    connect(this, SIGNAL(mediaCursorChanged()), orthoView, SLOT(updateView()));
...
void MainWindow::initPlaybackView() {
    ...
    connect(m_ui->actionShowPlaybackView, SIGNAL(toggled(bool)), dockWidget,
            SLOT(setVisible(bool)));
    connect(dockWidget, SIGNAL(visibilityChanged(bool)), m_ui->actionShowPlaybackView,
            SLOT(setChecked(bool)));

    connect(this, SIGNAL(dataChanged()), playbackView, SLOT(updateView()));
    connect(this, SIGNAL(rangeChanged()), playbackView, SLOT(updateView()));
...
void MainWindow::initLogbookView() {
    ...
    connect(m_ui->actionShowLogbookView, SIGNAL(toggled(bool)), dockWidget, SLOT(setVisible(bool)));
    connect(dockWidget, SIGNAL(visibilityChanged(bool)), m_ui->actionShowLogbookView,
            SLOT(setChecked(bool)));

    connect(this, SIGNAL(databaseChanged()), logbookView, SLOT(updateView()));
...
void MainWindow::initSimulationView() {
    ...
    connect(this, SIGNAL(dataChanged()), simulationView, SLOT(updateView()));
    connect(this, SIGNAL(mediaCursorChanged()), simulationView, SLOT(updateView()));
    connect(this, SIGNAL(mediaPaused()), simulationView, SLOT(pauseMedia()));

    connect(m_ui->actionShowSimulationView, SIGNAL(toggled(bool)), dockWidget,
            SLOT(setVisible(bool)));
    connect(m_ui->actionShowSimulationView, SIGNAL(toggled(bool)), this,
            SIGNAL(mediaCursorChanged()));
    connect(dockWidget, SIGNAL(visibilityChanged(bool)), m_ui->actionShowSimulationView,
            SLOT(setChecked(bool)));
    connect(dockWidget, SIGNAL(visibilityChanged(bool)), this, SIGNAL(mediaCursorChanged()));
...
DataPoint MainWindow::interpolateDataT(double t) {
...
int MainWindow::findIndexBelowT(double t) {
    int below = -1;
...
int MainWindow::findIndexAboveT(double t) {
    int below = -1;
    int above = m_data.size();
...
void MainWindow::on_actionImport_triggered() {
    ...
    QStringList fileNames = QFileDialog::getOpenFileNames(
        this, tr("Import Tracks"), settings.value("folder").toString(), tr("CSV Files (*.csv)"));
...
void MainWindow::on_actionImportFolder_triggered() {
    ...
    QString folderName = QFileDialog::getExistingDirectory(
        this, tr("Import Folder"), settings.value("folder").toString(), QFileDialog::ShowDirsOnly);

    // Import each file
    importFolder(folderName);
}

void MainWindow::importFolder(QString folderName) {
    QDir dir(folderName);

    // Import each file in this folder
    foreach (QString fileName, dir.entryList(QStringList() << "*.csv", QDir::Files, QDir::Name)) {
        importFile(dir.absoluteFilePath(fileName));
    }

    // Follow subfolders
    foreach (QString child, dir.entryList(QDir::Dirs | QDir::NoDotAndDotDot, QDir::Name)) {
...
void MainWindow::importFile(QString fileName) {
    ...
    if (!query.exec(QString("select * from files where file_name='%1'").arg(uniqueName))) {
        QSqlError err = query.lastError();
        QMessageBox::critical(0, tr("Query failed"), err.text());
        return;
    }
    ...
    if (!isPresent) {
        if (!query.exec(QString("insert into files (file_name) values ('%1')").arg(uniqueName))) {
            QSqlError err = query.lastError();
            QMessageBox::critical(0, tr("Query failed"), err.text());
        }
    }
...
void MainWindow::importFromDatabase(const QString& uniqueName) {
    // Get name of file in database
    QString newName = QString("FlySight/Tracks/%1.csv").arg(uniqueName);
    QString newPath = QDir(mDatabasePath).filePath(newName);
...
void MainWindow::setTrackName(const QString& trackName) {
    mTrackName = trackName;
    emit databaseChanged();
}

void MainWindow::setSelectedTracks(QVector<QString> tracks) {
    mSelectedTracks = tracks;
    m_ui->actionDeleteTrack->setEnabled(!mSelectedTracks.empty());
}

void MainWindow::setTrackDescription(const QString& trackName, const QString& description) {
    QSqlQuery query(mDatabase);
...
QString MainWindow::trackDescription(const QString& trackName) {
    QSqlQuery query(mDatabase);
...
QDateTime MainWindow::trackStartTime(const QString& trackName) {
    QSqlQuery query(mDatabase);
    ...
    if (!query.next())
        ...
        return QDateTime::fromString(query.value(0).toString(), "yyyy-MM-dd HH:mm:ss.zzz");
}
...
bool MainWindow::trackChecked(const QString& trackName) const {
...
void MainWindow::initTime(DataPoints& data) {
...
void MainWindow::initExit(DataPoints& data, QString trackName, bool initDatabase) {
    QString value;
    qint64 start;
    if (getDatabaseValue(trackName, "exit", value)) {
        start = QDateTime::fromString(value, Qt::ISODate).toMSecsSinceEpoch();
    }
    else {
...
void MainWindow::initAltitude(DataPoints& data, QString trackName, bool initDatabase) {
    QString value;
    double ground;
...
void MainWindow::initAcceleration(DataPoints& data) {
    for (int i = 0; i < data.size(); ++i) {
        DataPoint& dp = data[i];
...
void MainWindow::updateVelocity(DataPoints& data, QString trackName, bool initDatabase) {
    if (dataSize() == 0)
        return;
    ...
    for (int i = 0; i < data.size(); ++i) {
        ...
        // Adjust heading
        if (!firstHeading) {
            while (dp.heading < prevHeading - 180)
                dp.heading += 360;
            while (dp.heading >= prevHeading + 180)
                dp.heading -= 360;
        }
...
void MainWindow::initAerodynamics(DataPoints& data) {
    for (int i = 0; i < data.size(); ++i) {
        DataPoint& dp = data[i];
        ...
        const double airPressure = SL_PRESSURE * pow(1 - LAPSE_RATE * dp.hMSL / SL_TEMP,
...
double MainWindow::getSlope(const int center, double (*value)(const DataPoint&)) const {
...
double MainWindow::getDistance(const DataPoint& dp1, const DataPoint& dp2) {
...
double MainWindow::getBearing(const DataPoint& dp1, const DataPoint& dp2) {
    if (!mWindAdjustment && dp1.hasGeodetic && dp2.hasGeodetic) {
        const Geodesic& geod = Geodesic::WGS84();
        double azi1, azi2;
...
void MainWindow::setMediaCursor(double mediaCursor) {
    mMediaCursor = mediaCursor;

    emit mediaCursorChanged();
}

void MainWindow::mediaCursorAddRef(QObject* parent) {
    mMediaCursorRef.insert(parent);

    emit mediaCursorChanged();
}

void MainWindow::mediaCursorRemoveRef(QObject* parent) {
...
void MainWindow::initRange(QString trackName) {
    // Clear zoom stack
...
void MainWindow::updateLeftActions() {
    ...
    m_ui->actionHorizontalAccuracy->setChecked(
        m_ui->plotArea->plotVisible(DataPlot::HorizontalAccuracy));
    m_ui->actionVerticalAccuracy->setChecked(
        m_ui->plotArea->plotVisible(DataPlot::VerticalAccuracy));
    m_ui->actionSpeedAccuracy->setChecked(m_ui->plotArea->plotVisible(DataPlot::SpeedAccuracy));
    m_ui->actionNumberOfSatellites->setChecked(
        m_ui->plotArea->plotVisible(DataPlot::NumberOfSatellites));
...
void MainWindow::on_actionImportGates_triggered() {
    QStringList fileNames =
        QFileDialog::getOpenFileNames(this, tr("Import Gates"), "", tr("CSV Files (*.csv)"));
...
void MainWindow::on_actionPreferences_triggered() {
    ...
    if (dlg.exec() == QDialog::Accepted) {
        ...
        if (m_minDrag != dlg.minDrag() || m_minLift != dlg.minLift() ||
            m_maxLift != dlg.maxLift() || m_maxLD != dlg.maxLD()) {
        ...
        if (mGroundReference != dlg.groundReference() || mFixedReference != dlg.fixedReference()) {
...
void MainWindow::on_actionImportVideo_triggered() {
    ...
    if (!fileName.isEmpty()) {
        ...
        connect(this, SIGNAL(dataChanged()), videoView, SLOT(updateView()));
        connect(this, SIGNAL(mediaCursorChanged()), videoView, SLOT(updateView()));
        connect(this, SIGNAL(mediaPaused()), videoView, SLOT(pauseMedia()));
...
void MainWindow::on_actionExportKML_triggered() {
    ...
    QString fileName =
        QFileDialog::getSaveFileName(this, tr("Export KML"), rootFolder, tr("KML Files (*.kml)"));
    ...
    if (!fileName.isEmpty()) {
        ...
        if (description.isEmpty()) {
            ...
            description +=
                QString("%1").arg(startTime.toUTC().time().msec(), 3, 10, QChar('0')) + "Z";
        }
        ...
        for (int i = 0; i < dataSize(); ++i) {
            ...
            if (lower <= dp.t && dp.t <= upper) {
                ...
                for (int j = 0; j < DataPlot::yaLast; ++j) {
                    if (!m_ui->plotArea->yValue(j)->visible())
                        continue;
                    stream << QString(",%1").arg(m_ui->plotArea->yValue(j)->value(dp, m_units), 0,
                                                 'f');
                }
                stream << endl;
            }
...
void MainWindow::on_actionExportTrack_triggered() {
    ...
    if (!fileName.isEmpty()) {
        ...
        // Write header
        stream << "time,lat,lon,hMSL,velN,velE,velD,hAcc,vAcc,sAcc,heading,cAcc,gpsFix,numSV"
               << endl;
...
QString MainWindow::dateTimeToUTC(const QDateTime& dt) {
    QString ret;
...
void MainWindow::updateGround(DataPoints& data, double ground) {
    for (int i = 0; i < data.size(); ++i) {
        DataPoint& dp = data[i];
        dp.z = dp.hMSL - ground;
    }
...
bool MainWindow::getDatabaseValue(QString trackName, QString column, QString& value) {
    QSqlQuery query(mDatabase);

    // Read value from database
    if (!query.exec(
            QString("select %1 from files where file_name='%2'").arg(column).arg(trackName))) {
        QSqlError err = query.lastError();
        QMessageBox::critical(0, tr("Query failed"), err.text());
        return false;
    }
...
void MainWindow::setScoringVisible(bool visible) {
    emit dataChanged();
}

void MainWindow::setMinDrag(double minDrag) {
    m_minDrag = minDrag;

    emit aeroChanged();
}

void MainWindow::setMaxLift(double maxLift) {
    m_maxLift = maxLift;

    emit aeroChanged();
}

void MainWindow::setMaxLD(double maxLD) {
    m_maxLD = maxLD;
...
void MainWindow::setLineThickness(double width) {
...
void MainWindow::getWindSpeedDirection(QString trackName, double* windSpeed,
...
void MainWindow::on_actionDeleteTrack_triggered() {
    ...
    if (QMessageBox::question(0, tr("FlySight"), message) != QMessageBox::Yes)
        return;
...
void MainWindow::setScoringMode(ScoringMode mode) {
    mScoringMode = mode;
    emit dataChanged();
}

void MainWindow::prepareDataPlot(DataPlot* plot) {
...
void MainWindow::prepareMapView(MapView* view) {
    if (mScoringView->isVisible()) {
        mScoringMethods[mScoringMode]->prepareMapView(view);
    }
}

bool MainWindow::updateReference(double lat, double lon) {
    if (mScoringView->isVisible()) {
