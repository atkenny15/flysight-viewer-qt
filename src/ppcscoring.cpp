...
PPCScoring::PPCScoring(MainWindow* mainWindow) :
    ScoringMethod(mainWindow), mMainWindow(mainWindow), mMode(Time), mWindowTop(2500),
    mWindowBottom(1500), mDrawLane(false), mEndLatitude(51.0500), mEndLongitude(-114.0667),
    mLaneWidth(600) {
}
...
void PPCScoring::setDrawLane(bool drawLane) {
    mDrawLane = drawLane;
    emit scoringChanged();
}

void PPCScoring::setEnd(double endLatitude, double endLongitude) {
    mEndLatitude = endLatitude;
    mEndLongitude = endLongitude;
    emit scoringChanged();
}

void PPCScoring::setMode(Mode mode) {
    mMode = mode;
    emit scoringChanged();
}

void PPCScoring::setWindow(double windowBottom, double windowTop) {
    mWindowBottom = windowBottom;
    mWindowTop = windowTop;
    emit scoringChanged();
}

double PPCScoring::score(const MainWindow::DataPoints& result) {
...
QString PPCScoring::scoreAsText(double score) {
    switch (mMode) {
        case Time:
...
void PPCScoring::prepareDataPlot(DataPlot* plot) {
...
void PPCScoring::prepareMapView(MapView* view) {
    // Return now if plot empty
    if (mMainWindow->dataSize() == 0)
        return;
...
void PPCScoring::splitLine(QList<QVariant>& data, double startLat, double startLon, double endLat,
...
bool PPCScoring::getWindowBounds(const MainWindow::DataPoints& result, DataPoint& dpBottom,
                                 DataPoint& dpTop) {
...
bool PPCScoring::updateReference(double lat, double lon) {
