...
OrthoView::OrthoView(QWidget* parent) :
    QCustomPlot(parent), mMainWindow(0), m_pan(false), m_azimuth(-PI / 2), m_elevation(PI / 2),
    m_scale(1) {
...
void OrthoView::mousePressEvent(QMouseEvent* event) {
...
void OrthoView::mouseReleaseEvent(QMouseEvent* event) {
    m_pan = false;
    QCustomPlot::mouseReleaseEvent(event);
}

void OrthoView::mouseMoveEvent(QMouseEvent* event) {
    if (m_pan) {
...
void OrthoView::wheelEvent(QWheelEvent* event) {
    // Adjust scale
...
void OrthoView::updateView() {
    ...
    QVector3D up(-sin(m_elevation) * cos(m_azimuth), -sin(m_elevation) * sin(m_azimuth),
                 cos(m_elevation));
    QVector3D bk(cos(m_elevation) * cos(m_azimuth), cos(m_elevation) * sin(m_azimuth),
                 sin(m_elevation));
    ...
    QVector3D mid((uMin + uMax) / 2, (vMin + vMax) / 2, (wMin + wMax) / 2);
    ...
    setViewRange(xMid - rMax / m_scale, xMid + rMax / m_scale, yMid - rMax / m_scale,
                 yMid + rMax / m_scale);
...
void OrthoView::addOrientation() {
    ...
    // Transformed basis
    QVector3D o(-rt.x() - rt.y() - rt.z(), -up.x() - up.y() - up.z(), -bk.x() - bk.y() - bk.z());
    QVector3D x(rt.x() - rt.y() - rt.z(), up.x() - up.y() - up.z(), bk.x() - bk.y() - bk.z());
    QVector3D y(-rt.x() + rt.y() - rt.z(), -up.x() + up.y() - up.z(), -bk.x() + bk.y() - bk.z());
    QVector3D z(-rt.x() - rt.y() + rt.z(), -up.x() - up.y() + up.z(), -bk.x() - bk.y() + bk.z());

    // Corner in plot space
    QPointF corner(xAxis->range().upper - 10 * valPerMM, yAxis->range().upper - 10 * valPerMM);
...
void OrthoView::setViewRange(double xMin, double xMax, double yMin, double yMax) {
    QPainter painter(this);
