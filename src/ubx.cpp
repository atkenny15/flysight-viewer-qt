...
const uint16_t UBX::mSasTable[12] = { 1024, 1077, 1135, 1197, 1265, 1338,
                                      1418, 1505, 1600, 1704, 1818, 1944 };

UBX::UBX(Config& config, Tone& tone) : mConfig(config), mTone(tone) {
    mCurSpeech = 0;
...
char* UBX::writeInt32ToBuf(char* ptr, int32_t val, int8_t dec, int8_t dot, char delimiter) {
...
void UBX::setTone(int32_t val_1, int32_t min_1, int32_t max_1, int32_t val_2, int32_t min_2,
                  int32_t max_2) {
#define UNDER(val, min, max) ((min < max) ? (val <= min) : (val >= min))
#define OVER(val, min, max) ((min < max) ? (val >= max) : (val <= max))

    if (val_1 != UBX_INVALID_VALUE && val_2 != UBX_INVALID_VALUE) {
        if (UNDER(val_2, min_2, max_2)) {
            ...
            mTone.setRate(mConfig.UBX_min_rate + (mConfig.UBX_max_rate - mConfig.UBX_min_rate) *
                                                     (val_2 - min_2) / (max_2 - min_2));
        }
...
void UBX::getValues(UBX_saved_t* current, uint8_t mode, int32_t* val, int32_t* min, int32_t* max) {
    ...
    switch (mode) {
        ...
        case 5: // Direction to destination
            ...
            if ((Nav::calcDistance(current->lat, current->lon, mConfig.UBX_lat, mConfig.UBX_lon) <
                 mConfig.UBX_max_dist) ||
                (mConfig.UBX_max_dist == 0)) {
                // check if above height tone should be silenced
                if ((current->hMSL > (mConfig.UBX_end_nav + mConfig.UBX_dz_elev)) ||
                    (mConfig.UBX_end_nav == 0)) {
                    tVal =
                        Nav::calcDirection(mConfig, current->lat, current->lon, current->heading);
                    // check if heading not within UBX_min_angle deg of bearing or tones needed for
                    // other measurement
                    if ((ABS(tVal) > mConfig.UBX_min_angle) || (mConfig.UBX_mode_2 != 5) ||
                        (mConfig.UBX_min_angle == 0)) {
                        *min = -180;
                        *max = 180;
        ...
        case 7: // Direction to bearing
            ...
            if ((current->hMSL > (mConfig.UBX_end_nav + mConfig.UBX_dz_elev)) ||
                ...
                // check if heading not within UBX_min_angle deg of bearing or tones needed for
                // other measurement
                if ((ABS(tVal) > mConfig.UBX_min_angle) || (mConfig.UBX_mode_2 != 7) ||
                    (mConfig.UBX_min_angle == 0)) {
                    *min = -180;
                    *max = 180;
                    // manipulate tone so biggest change is at desired heading
...
char* UBX::numberToSpeech(int32_t number, char* ptr) {
    // Adapted from
...
void UBX::speakValue(UBX_saved_t* current) {
    ...
    switch (mConfig.UBX_speech[mCurSpeech].mode) {
        ...
        case 2: // Glide ratio
            if (current->velD != 0) {
                mSpeechPtr = writeInt32ToBuf(
                    mSpeechPtr, 100 * (int32_t)current->gSpeed / current->velD, 2, 1, 0);
            }
            else {
                *(--mSpeechPtr) = 0;
            }
        ...
        case 3: // Inverse glide ratio
            if (current->gSpeed != 0) {
                mSpeechPtr = writeInt32ToBuf(
                    mSpeechPtr, 100 * (int32_t)current->velD / current->gSpeed, 2, 1, 0);
            }
            else {
                *(--mSpeechPtr) = 0;
            }
        ...
        case 6: // Distance to destination
            decimals = 1;
            tVal = Nav::calcDistance(current->lat, current->lon, mConfig.UBX_lat,
                                     mConfig.UBX_lon); // returns metres
            switch (mConfig.UBX_speech[mCurSpeech].units) {
                case UBX_UNITS_KMH:
                    tVal = tVal / 10;
                    break;
        ...
        case 11: // Dive angle
            mSpeechPtr = writeInt32ToBuf(
                mSpeechPtr, 100 * atan2(current->velD, current->gSpeed) / M_PI * 180, 2, 1, 0);
            break;
        case 12: // Altitude
...
void UBX::updateAlarms(UBX_saved_t* current) {
    ...
    if (mPrevFlags & UBX_HAS_FIX) {
        ...
        if ((mConfig.UBX_alt_step > 0) && (i == mConfig.UBX_alarms.length()) &&
            (mPrevHMSL - mConfig.UBX_dz_elev >= UBX_ALT_MIN * 1000) && (*mSpeechPtr == 0) &&
            !(mFlags & UBX_SAY_ALTITUDE) && !suppress_alt) {
...
void UBX::updateTones(UBX_saved_t* current) {
    ...
    switch (mConfig.UBX_mode_2) {
        case 5:                        // Direction to destination
            if (mConfig.UBX_mode == 5) // no need to re-calculate direction
            {
                ...
                val_2 =
                    ABS(Nav::calcDirection(mConfig, current->lat, current->lon, current->heading));
        ...
        case 9: // Change in value 1
            ...
            if (mVal[0] != UBX_INVALID_VALUE && mVal[1] != UBX_INVALID_VALUE &&
                mVal[2] != UBX_INVALID_VALUE && max_1 != min_1) {
    ...
    if (!mSuppressTone) {
        if (ABS(current->velD) >= mConfig.UBX_threshold &&
            ...
            if (mConfig.UBX_sp_rate != 0 && mConfig.UBX_speech.length() != 0 &&
                mSpCounter >= mConfig.UBX_sp_rate && (*mSpeechPtr == 0) &&
...
void UBX::receiveMessage(const DataPoint& dp) {
    UBX_saved_t current;
...
void UBX::task() {
    if (*mSpeechPtr) {
        ...
        if ((mFlags & UBX_SAY_ALTITUDE) && (mFlags & UBX_HAS_FIX) && (mFlags & UBX_VERTICAL_ACC) &&
            mTone.isIdle()) {
            mFlags &= ~UBX_SAY_ALTITUDE;
            mSpeechPtr = mSpeechBuf;
            ...
            if (mConfig.UBX_alt_units == UBX_UNITS_METERS) {
                ...
                *(mSpeechPtr++) = 'm';
                ...
                mSpeechPtr =
                    numberToSpeech((mPrevHMSL - mConfig.UBX_dz_elev) * 10 / 3048, mSpeechPtr);
                *(mSpeechPtr++) = 'f';
            }
