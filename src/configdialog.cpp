...
ConfigDialog::ConfigDialog(MainWindow* mainWindow) : QDialog(mainWindow), ui(new Ui::ConfigDialog) {
    ui->setupUi(this);
    ...
    // Add page
    ui->contentsWidget->addItems(QStringList() << tr("General"));
    ui->contentsWidget->addItems(QStringList() << tr("Import"));
    ui->contentsWidget->addItems(QStringList() << tr("Aerodynamics"));
    ui->contentsWidget->addItems(QStringList() << tr("Plots"));

    // Add units
    ui->unitsCombo->addItems(QStringList() << tr("Metric") << tr("Imperial"));

    // Update plot widget
    updatePlots();

    // Update plots when units are changed
    connect(ui->unitsCombo, SIGNAL(currentIndexChanged(int)), this, SLOT(updatePlots()));
    ...
    connect(ui->autoReferenceButton, SIGNAL(clicked(bool)), this, SLOT(updateReference()));
    connect(ui->fixedReferenceButton, SIGNAL(clicked(bool)), this, SLOT(updateReference()));
...
void ConfigDialog::on_browseButton_clicked() {
    ...
    QString rootFolder = QFileDialog::getExistingDirectory(this, "", ui->logbookFolderEdit->text());
...
void ConfigDialog::updatePlots() {
    ...
    ui->plotTable->setHorizontalHeaderLabels(QStringList()
                                             << tr("Colour") << tr("Minimum") << tr("Maximum"));
...
void ConfigDialog::setUnits(PlotValue::Units units) {
...
void ConfigDialog::setGroundReference(MainWindow::GroundReference ref) {
...
void ConfigDialog::setFixedReference(double elevation) {
    ui->fixedReferenceEdit->setText(QString("%1").arg(elevation));
}

MainWindow::GroundReference ConfigDialog::groundReference() const {
    if (ui->autoReferenceButton->isChecked())
        return MainWindow::Automatic;
    else
        return MainWindow::Fixed;
}

void ConfigDialog::setMass(double mass) {
...
void ConfigDialog::setPlanformArea(double area) {
...
void ConfigDialog::setMinDrag(double minDrag) {
...
void ConfigDialog::setMinLift(double minLift) {
...
void ConfigDialog::setMaxLift(double maxLift) {
...
void ConfigDialog::setMaxLD(double maxLD) {
...
void ConfigDialog::setSimulationTime(int simulationTime) {
...
void ConfigDialog::setWindSpeed(double speed) {
...
void ConfigDialog::setWindUnits(QString units) {
    ui->windUnitsLabel->setText(units);
}

void ConfigDialog::setWindDirection(double dir) {
...
void ConfigDialog::setUseDatabase(bool use) {
...
void ConfigDialog::setDatabasePath(QString databasePath) {
    // Update database path
